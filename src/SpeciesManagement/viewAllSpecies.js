...
import ViewOneSpecies from "./viewOneSpecies.js";
...
function ViewAllSpecies() {
  ...
  useEffect(() => {
    const fetchSpecies = async () => {
      try {
        const res = await axios.get("http://localhost:8081/species/getAllSpecies");
        setSpecies(res.data);
      } catch (error) {
  ...
  const handleExport = () => {
    ...
    doc.setTextColor(14, 108, 145); // AquaShield blue
    doc.text("AquaShield", 14, 20);

    doc.setFontSize(16);
    doc.setFont("helvetica", "bold");
    doc.setTextColor(0, 0, 0);
    doc.text("Species Report", 14, 30);
    ...
    const tableRows = filteredSpecies.map((sp) => [
      sp.CommonName || "-",
      sp.ScientificName || "-",
      sp.ProtectionLevel || "-",
    ...
    autoTable(doc, {
      ...
      headStyles: { fillColor: [23, 169, 211] }, // header blue
    });
