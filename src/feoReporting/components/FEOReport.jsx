...
import incidentImg from '../../reporting/Image/image.png'
...
const FeoReport = () => {
  ...
  useEffect(() => {
    const getReports = () => {
      axios
        .get("http://localhost:8081/api/report/getAllReports")
        .then((res) => {
  ...
  const handleStatusChange = async (reportId, status) => {
    ...
    console.log("Your in action button");
  setLoading(true);
  try {
    const response = await axios.put(
      `http://localhost:8081/api/report/reportAction/${reportId}`,
      { status },
      ...
      {selectReport?(
        ...
        <div className="container mx-auto p-4">
          <button 
            ...
            className="mb-4 flex items-center text-cyan-700 hover:text-cyan-900"
          >
        ...
        <ReportInfo report={selectReport} onClose={()=>selectReport(null)}/>
                      ...
                      {report.status === "PENDING" && (
                        <div className="flex justify-end space-x-2">
                          <button className="px-3 py-1 bg-blue-500 text-white rounded text-sm hover:bg-blue-600"
                           onClick={(e) => {
                                e.stopPropagation();
                                handleStatusChange(report._id,'VERIFIED')
                            }}>
                            Accept
                          </button>
                          <button 
                            ...
                            onClick={(e) => {
                                e.stopPropagation();
                                handleStatusChange(report._id,'REJECT')
                            }}
                          >
                            Reject
                          </button>
